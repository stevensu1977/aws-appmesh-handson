...
RESOURCE_PATH="../resource/auto_inject"
...
aws appmesh create-virtual-node  --region ${AWS_REGION} --mesh-name ${APPMESH_NAME}  --cli-input-json file://${MESH_RESOURCE_PATH}/V15/customer/customer-v15-vn.json


printf "${GREEN}AWS AppMesh | region: [ ${AWS_REGION} ], appmesh name:[ ${APPMESH_NAME} ] , update route to new virtual-node .....${NC}\n"
aws appmesh update-route  --region ${AWS_REGION} --mesh-name ${APPMESH_NAME} --cli-input-json file://${MESH_RESOURCE_PATH}/V15/customer/customer-canary.json
